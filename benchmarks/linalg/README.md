...
`matrix_sqrt_temp_benchmark.py` compares two ATen ops for the principal square root of Hermitian PSD matrices, on speed and accuracy:

- `matrix_sqrth` uses the eigh path (`?syevd`/`?heevd` from LAPACK, symmetry-exploiting).
- `matrix_sqrt_temp` uses generic Schur path (`?gees` + Bjorck-Hammarling, `?trsyl`).
...
sudo OMP_NUM_THREADS=1 VECLIB_MAXIMUM_THREADS=1 nice -n -20 \
  .venv/bin/python -m benchmarks.linalg.matrix_sqrt_temp_benchmark \
  --label 1-core --out benchmarks/linalg/matrix_sqrt_temp_results_1core.md
```
