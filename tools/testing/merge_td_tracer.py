...
def merge_td_tracer_outputs(
    ...
    *,
    test_workflow_successful: bool = True,
) -> dict[str, Any]:
    if not run_id:
    ...
    successful = (
        has_all_shards
        and test_workflow_successful
        and all(output["successful"] for output in outputs)
    )
    return {
...
def main() -> None:
    ...
    parser.add_argument("--test-workflow-result", required=True)
    ...
    merged = merge_td_tracer_outputs(
        ...
        args.expected_shard,
        test_workflow_successful=args.test_workflow_result == "success",
    )
