...
class TestRecheckAutotuneCache(TestCase):
    ...
    @skipUnless(HAS_GPU_AND_TRITON, "requires gpu and triton")
    def test_recheck_cache_hit_sets_found_by_coordesc_true(self):
        """
        On a cache hit, found_by_coordesc is always set to True on the
        compile result's config to prevent coordinate descent from re-running,
        even if the cached config was not originally found by coordesc.
        """
        cfg = triton_config({"x": 16}, 64)
        cfg.found_by_coordesc = False
        compile_result = self._make_compile_result(cfg)
