...
_CACHES = (
    ...
    # create_if_missing so we can test the component even if we don't have redis
    # installed.
    CacheDecl("redis.Redis", _MockRedisRemoteCache, create_if_missing=True),
)
...
class PatchCaches(contextlib.AbstractContextManager):
    ...
    def __init__(self):
        ...
        for decl in self.get_caches():
            self._contexts.append(patch(decl.qname, decl.f or decl.cls))
