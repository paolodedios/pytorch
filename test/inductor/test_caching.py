...
class InterfacesTest(TestMixin, TestCase):
    ...
    def test_persistent_memoizer_loads_from_root_when_sub_dir_empty(self) -> None:
        ...
        singletons (not from any nested structure).
        """
        import json
        import os
        import tempfile
    ...
    def test_memoizer_dump_to_disk_creates_json_file(self) -> None:
        ...
        for entry in data["singletons"].values():
    ...
    def test_memoizer_dump_to_disk_merges_with_existing(self) -> None:
        ...
        self.assertEqual(data["cache_size"], 2)
        self.assertEqual(len(data["singletons"]), 2)
...
class TunedMMEncodingTest(TestMixin, TestCase):
    ...
    def test_tuned_mm_encoding_multi_template_buffer(self) -> None:
        ...
        for cache_entry in cache._memory.values():
            ...
            # Verify encoded_result structure (TunedKernelEncodedResult)
            # For MultiTemplateBuffer, _type should be "multi_template_buffer" or "unknown"
            self.assertIn("_type", cache_entry.encoded_result)
            result_type = cache_entry.encoded_result["_type"]
            self.assertIn(result_type, ["multi_template_buffer", "unknown"])
