...
class DeferredRecordingTest(TestMixin, TestCase):
    ...
    @set_caching_module_enabled(False)
    def test_deferred_recording_disabled_caching(self) -> None:
        """Test that deferred recording has no effect when caching is disabled.

        Verifies that when caching is disabled, the encoder is not called
        and the function just returns the result normally.
        """
        # Setup
        memoizer = Memoizer()
        ...
        # Assert: result is correct and encoder was not called
        self.assertEqual(result, 10)
