...
class TestSelectAlgorithm(TestCase):
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose1d(self):
        @torch.compile
        def foo(x, w, b):
            return aten.convolution(
                x,
        ...
        foo(
            torch.randn(2, 16, 20, device=GPU_TYPE),
            torch.randn(16, 32, 3, device=GPU_TYPE),
            torch.randn(32, device=GPU_TYPE),
        )
        if not torch.version.hip:
            self.assertEqual(counters["inductor"]["select_algorithm_autotune"], 1)
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose1d_no_bias(self):
        @torch.compile
        def foo(x, w):
            return aten.convolution(
                x,
        ...
        foo(
            torch.randn(4, 16, 50, device=GPU_TYPE),
            torch.randn(16, 32, 3, device=GPU_TYPE),
        )
        if not torch.version.hip:
            self.assertEqual(counters["inductor"]["select_algorithm_autotune"], 1)
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose1d_groups(self):
        @torch.compile
        def foo(x, w, b):
            return aten.convolution(
                x,
        ...
        foo(
            torch.randn(2, 32, 30, device=GPU_TYPE),
            torch.randn(32, 8, 5, device=GPU_TYPE),
            torch.randn(32, device=GPU_TYPE),
        )
        if not torch.version.hip:
            self.assertEqual(counters["inductor"]["select_algorithm_autotune"], 1)
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose2d(self):
        @torch.compile
        def foo(x, w, b):
            return aten.convolution(
                x,
        ...
        foo(
            torch.randn(2, 16, 12, 12, device=GPU_TYPE),
            torch.randn(16, 32, 3, 3, device=GPU_TYPE),
            torch.randn(32, device=GPU_TYPE),
        )
        if not torch.version.hip:
            self.assertEqual(counters["inductor"]["select_algorithm_autotune"], 1)
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose2d_no_bias(self):
        @torch.compile
        def foo(x, w):
            return aten.convolution(
                x,
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose2d_groups(self):
        @torch.compile
        def foo(x, w, b):
            return aten.convolution(
                x,
        ...
        foo(
            torch.randn(2, 32, 16, 16, device=GPU_TYPE),
            torch.randn(32, 8, 3, 3, device=GPU_TYPE),
            torch.randn(32, device=GPU_TYPE),
        )
        if not torch.version.hip:
            self.assertEqual(counters["inductor"]["select_algorithm_autotune"], 1)
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose3d(self):
        @torch.compile
        def foo(x, w, b):
            return aten.convolution(
                x,
        ...
        foo(
            torch.randn(2, 16, 4, 4, 4, device=GPU_TYPE),
            torch.randn(16, 32, 3, 3, 3, device=GPU_TYPE),
            torch.randn(32, device=GPU_TYPE),
        )
        if not torch.version.hip:
            self.assertEqual(counters["inductor"]["select_algorithm_autotune"], 1)
    ...
    @expectedFailureDynamicWrapper
    @patches
    def test_convolution_transpose3d_no_bias(self):
        @torch.compile
        def foo(x, w):
            return aten.convolution(
                x,
