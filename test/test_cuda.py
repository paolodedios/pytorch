...
""",
    ...
    def test_graph_rng_concurrent_captures(self):
        ...
        # Verify streams are no longer capturing
        with torch.cuda.stream(s0):
            self.assertFalse(torch.cuda.is_current_stream_capturing())
        with torch.cuda.stream(s1):
            self.assertFalse(torch.cuda.is_current_stream_capturing())
        ...
        # Replay both graphs and verify they produced random values
        buf0.zero_()
        buf1.zero_()
        g0.replay()
        g1.replay()
    ...
    def test_graph_rng_after_failed_capture(self):
        ...
        with torch.cuda.stream(stream):
            ...
            with self.assertRaises(RuntimeError):
                (x + 1).item()
            # capture_end should fail because the capture is in error state
            with self.assertRaises(RuntimeError):
                graph.capture_end()
    ...
    def test_graph_rng_shared_pool_replay_matches_eager(self):
        ...
        # Replay and verify results match eager execution
        buf0.zero_()
        buf1.zero_()
        g0.replay()
        g1.replay()
    ...
    def test_memory_stats_of_multiple_generators_and_graphs(self):
        ...
        def test(num_graphs, num_generators):
            ...
            default_generator = torch.cuda.default_generators[0]
            generators_states = [default_generator.graphsafe_get_state()]

            # Starts from 1 as one state is already added
            for _ in range(1, num_generators):
                state = default_generator.clone_state()
                generators_states.append(state)

            for graph in graphs:
                simple_graph_task(graph, default_generator, generators_states)
