...
default_atol = {torch.float16: 1e-3, torch.bfloat16: float('infinity'), torch.float32: 1e-5}
default_rtol = {torch.float16: 1e-3, torch.bfloat16: float('infinity'), torch.float32: 1.3e-6}
...
def run_comp_nocomp(
    ...
    if not (math.isinf(kwargs["atol"]) or math.isinf(kwargs["rtol"])):
