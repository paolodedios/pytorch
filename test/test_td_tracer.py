...
class TestTDTracerArtifacts(TestCase):
    ...
    def test_merge_ci_shard_outputs_use_latest_attempt_per_shard(self):
        with TemporaryDirectory() as tmp_dir:
            tmp_path = Path(tmp_dir)
            paths = []
            outputs = {
                "run-1-default-1": {"test": ["torch/old.py"]},
                "run-2-default-1": {"test": ["torch/new.py"]},
                "run-1-default-2": {"test": ["torch/other.py"]},
            }
            for run_id, coverage in outputs.items():
                path = tmp_path / f"{run_id}.json"
                _atomic_write_json(path, self._shard_output(run_id, coverage))
                paths.append(path)
        ...
        self.assertEqual(
            merged["coverage_by_test"],
            {"test": ["torch/new.py", "torch/other.py"]},
        )
        self.assertEqual(
            [participant["participant_id"] for participant in merged["participants"]],
    ...
    def test_merge_ci_shard_outputs_require_successful_test_workflow(self):
        with TemporaryDirectory() as tmp_dir:
            path = Path(tmp_dir) / "shard.json"
            _atomic_write_json(
                path,
            ...
            merged = merge_td_tracer_outputs(
                ...
                1,
                {"default-1"},
                test_workflow_successful=False,
            )
