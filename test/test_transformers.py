...
class TestSDPAXpuOnly(NNTestCase):
    ...
    def test_onednn_attention_unaligned_attn_mask(self, device, dtype, train):
        ...
        q = make_unaligned(qkv_numel).view(batch_size, q_size, n_head, head_dim).transpose(1, 2)
        k = make_unaligned(qkv_numel).view(batch_size, q_size, n_head, head_dim).transpose(1, 2)
        v = make_unaligned(qkv_numel).view(batch_size, q_size, n_head, head_dim).transpose(1, 2)
