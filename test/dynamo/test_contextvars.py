...
class TestContextVars(TestCase):
    ...
    def test_custom_module_setattr_graph_breaks(self):
        class CustomModule(types.ModuleType):
            def __init__(self):
                super().__init__("custom_contextvar_module")
                super().__setattr__(
                    "state",
                    contextvars.ContextVar("custom_module_state", default="before"),
                )
                super().__setattr__("target", 0)

            def __setattr__(self, name, value):
                if name == "target":
                    self.state.set("after")
        ...
        @torch.compile(backend=cnt)
        def fn(x):
            if mod.state.get() == "before":
                x = x + 1
            setattr(mod, "target", 1)  # noqa: B010
            if mod.state.get() == "after":
                x = x + 2
            return x
        ...
        self.assertEqual(fn(x), x + 3)
        self.assertEqual(cnt.frame_count, 2)
        self.assertEqual(mod.state.get(), "after")
