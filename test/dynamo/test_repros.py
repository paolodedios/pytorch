...
def forward(self, s77 : torch.SymInt, s27 : torch.SymInt, L_x_ : torch.Tensor):
    ...
    def test_mul_tensor_prop(self):
        # Test that mul_ propagates requires_grad correctly
        class MyFn(torch.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return torch.matmul(x, x.t())
    ...
    def test_sub_tensor_prop(self):
        # Test that sub_ propagates requires_grad correctly
        class MyFn(torch.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return torch.matmul(x, x.t())
    ...
    def test_div_tensor_prop(self):
        # Test that div_ propagates requires_grad correctly
        class MyFn(torch.autograd.Function):
            @staticmethod
            def forward(ctx, x):
                return torch.matmul(x, x.t())
        ...
        def inputs():
            ...
            x = torch.randn(10, 10)
            y = torch.randn(10, 10, requires_grad=True) + 1.0  # Avoid division by zero
            return x, y
