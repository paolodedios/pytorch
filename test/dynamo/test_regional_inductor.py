    ...
    def test_unbacked_expr_input(self, serialize):
        ...
        opt_fn = torch.compile(
            ...
            backend=aot_eager_regional_inductor(
                ...
                # TODO: there is a bug in default partitioner
                # https://github.com/pytorch/pytorch/issues/167012#issuecomment-3599692972
...
def forward(self, primals_0, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6, primals_7, primals_8):
    ...
    detach = torch.ops.aten.detach.default(getitem)
    detach_1 = torch.ops.aten.detach.default(getitem_1);  getitem_1 = None
    detach_2 = torch.ops.aten.detach.default(detach);  detach = None
    detach_3 = torch.ops.aten.detach.default(detach_1);  detach_1 = None
    return (getitem, primals_0, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6, primals_7, primals_8, detach_2, detach_3)""",  # noqa: B950
                ignore_comments=True,
                ignore_empty_lines=True,
            )
            # inductor compiled backward graph
            self.assertExpectedInline(
                captured_gms[1].code.strip(),
                """\
def forward(self, primals_0, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6, primals_7, primals_8, detach_2, detach_3, tangents_0):
    zeros = torch.ops.aten.zeros.default([1, 1, 768], dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0))
    fw_graph0 = self.fw_graph0
    joint_graph0 = self.joint_graph0
    mask_graph0 = self.mask_graph0
    flex_attention_backward = torch.ops.higher_order.flex_attention_backward(primals_0, primals_0, primals_0, detach_2, detach_3, tangents_0, zeros, fw_graph0, joint_graph0, (768, 768, primals_1, primals_2, primals_3, primals_4, primals_5, primals_6, primals_7, primals_8, 128, 128, mask_graph0), 0.125, {'BACKEND': 'AUTO', 'PRESCALE_QK': False, 'ROWS_GUARANTEED_SAFE': False, 'BLOCKS_ARE_CONTIGUOUS': False, 'WRITE_DQ': True, 'OUTPUT_LOGSUMEXP': True, 'OUTPUT_MAX': False}, (), ());  primals_0 = detach_2 = detach_3 = tangents_0 = zeros = fw_graph0 = joint_graph0 = primals_1 = primals_2 = primals_3 = primals_4 = primals_5 = primals_6 = primals_7 = primals_8 = mask_graph0 = None
