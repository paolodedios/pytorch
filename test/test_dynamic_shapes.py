...
def forward(self, x_1):
    ...
    def test_rename_unbacked_to_backed_dest_rewrites_unbacked_alias(self):
        shape_env = ShapeEnv()
        orig = shape_env.create_unbacked_symint().node.expr
        new = shape_env.create_unbacked_symint().node.expr
        ...
        shape_env._set_replacement(orig, backed, "test-setup")
        shape_env._rename_unbacked_to(orig, new)
        self.assertEqual(shape_env._find(orig), backed)
        self.assertEqual(shape_env._find(new), backed)
        self.assertEqual(shape_env._find(alias), backed)

    def test_rename_unbacked_to_skips_unreconcilable_alias(self):
        shape_env = ShapeEnv()
        orig = shape_env.create_unbacked_symint()
        new = shape_env.create_unbacked_symint()
    ...
    def test_rename_unbacked_to_two_backed_terminals_prefers_dest(self):
        shape_env = ShapeEnv()
        orig = shape_env.create_unbacked_symint().node.expr
        new = shape_env.create_unbacked_symint().node.expr
    ...
    def test_rename_unbacked_to_double_propagation_unifies(self):
