...
void DeviceMemoryAllocator::free(void* ptr) {
  ...
  if (it == allocation_sizes_.end()) {
    TORCH_WARN(
        ...
        " on device ",
        device_index_,
        " that is not tracked as an active allocation. "
        "This likely indicates a double-free.");
    return;
  }
...
void OpenRegDeviceAllocator::emptyCache(
    [[maybe_unused]] MempoolId_t mempool_id) {
