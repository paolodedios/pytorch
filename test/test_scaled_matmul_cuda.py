...
def scaled_mm_wrap(
    ...
    # XPU does not have the `torch._scaled_mm_v2` implementation, so fallback to the `_scaled_mm`
    if not wrap_v2 or torch.xpu.is_available():
...
class TestFP8Matmul(TestCase):
    ...
    def test_float8_basics(self, device) -> None:
        ...
        ctx = self.assertRaises(ValueError) if torch.version.hip is None and device == "cuda" else contextlib.nullcontext()
        with ctx:
    ...
    # XPU does not support rowwise scaling for now.
    @skipXPU
    @onlyOn(["cuda", "xpu"])
