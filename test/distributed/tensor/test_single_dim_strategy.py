...
class TestPropagateOutputSharding(TestCase):
    ...
    def _propagate(self, mesh, op, op_args, in_placements, op_kwargs=None):
        return DTensor._op_dispatcher.sharding_propagator.query_output_sharding(
            mesh, op, op_args, in_placements, op_kwargs
        )

    def test_query_output_sharding_mm(self):
    ...
    def test_query_output_sharding_mm_mismatch(self):
        mesh = DeviceMesh("cpu", torch.arange(self.world_size).reshape(2, 4))
        left_meta, right_meta = _get_mm_metas()
    ...
    def test_query_output_sharding_with_non_tensor_args(self):
        mesh = DeviceMesh("cpu", torch.arange(self.world_size))
        meta = TensorMeta(
    ...
    def test_query_output_sharding_multi_output(self):
        mesh = DeviceMesh("cpu", torch.arange(self.world_size))
        meta = TensorMeta(
            shape=torch.Size([8, 16]),
            stride=(16, 1),
    ...
    def test_query_output_sharding_sort(self):
        mesh = DeviceMesh("cpu", torch.arange(self.world_size))
        meta = TensorMeta(
            shape=torch.Size([8, 16]),
            stride=(16, 1),
    ...
    def test_query_output_sharding_split(self):
        mesh = DeviceMesh("cpu", torch.arange(self.world_size))
        meta = TensorMeta(
            shape=torch.Size([8, 16]),
            stride=(16, 1),
