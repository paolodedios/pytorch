...
class TestViewOps(DTensorContinuousTestBase):
    ...
    def test_view_groups_unbacked_sharding_propagation(self):
        ...
        # Strict view with a non-leftmost shard needs _StridedShard. Its
        # split_factor is semantic placement metadata, so an explicitly hinted
        # unbacked prefix can fill that int metadata without specializing the
        # tensor shape.
        u4_hint = fresh_sym()
        ...
        self.assertEqual(out_plc, [_StridedShard(0, split_factor=4), Replicate()])
        ...
        # Unflattening a _StridedShard should use the explicit hint to match
        # the Split prefix factor without guarding the symbolic dimension equal
        # to the hint.
        u5_hint = fresh_sym()
        ...
        rule = view_groups(from_shape, to_shape)
        input_placements = [_StridedShard(0, split_factor=8), Replicate()]
        inp_tgt, out_plc = propagate_shape_and_sharding(
            input_placements, from_shape, rule, mesh_sizes, strict_view=True
        )
        self.assertEqual(out_plc, [Shard(1), Replicate()])
