...
class ProcessGroupNCCLLargerScaleTest(MultiProcessTestCase):
    ...
    def test_split_group_partial_then_2d_teardown(self):
        ...
        # groups created by a partial split followed by a 2-D mesh split.
        # Root cause: _hash_ranks_to_str used len(_world.pg_names) as a uniqueness
        # suffix, which diverges between ranks for partial splits (some ranks don't
        # register the PG). This caused co-participants in the same NCCL communicator
        # to compute different pg_names, producing inconsistent teardown orderings
        # and circular waits in ncclCommFinalize.
        store = c10d.FileStore(self.file_name, self.world_size)
        device = torch.device(f"cuda:{self.rank}")
        pg = self._create_process_group_nccl(store, self.opts(), device_id=device)

        # Pattern 'a': partial 5-rank group (ranks 5,6,7 are non-members).
        partial = c10d.split_group(pg, [[0, 1, 2, 3, 4]])
        if partial is not None:
            dist.all_reduce(torch.ones(1, device=device), group=partial)

        # Pattern 'b': 4 rank-pairs, all 8 ranks are members of some subgroup.
        pairs = c10d.split_group(pg, [[0, 4], [1, 5], [2, 6], [3, 7]])
        if pairs is not None:
            dist.all_reduce(torch.ones(1, device=device), group=pairs)

        # Pattern 'c': 2-D split, all 8 ranks are members of some subgroup.
