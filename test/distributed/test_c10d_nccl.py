...
from torch.distributed.distributed_c10d import NCCL_SHRINK_ABORT
...
class ProcessGroupNCCLGroupTest(MultiProcessTestCase):
    ...
    @property
    def world_size(self):
        # Allow world_size to be configured via environment variable
        # This enables shrink tests to be run with WORLD_SIZE more than 2
        ...
        if env_world_size is not None:
            try:
                ...
                pass
        return 2  # Default value for backward compatibility
    ...
    @requires_nccl()
    @skip_but_pass_in_sandcastle_if(
        int(os.environ.get("WORLD_SIZE", "1")) < 2, "Test requires world size >= 2"
    )
    ...
    def test_shrink_group_backend_properties(self):
        ...
        new_rank = self._validate_shrunk_group(shrunk_pg, expected_size, test_name)
    ...
    def test_shrink_group_performance(self):
        ...
        if not is_excluded:
            ...
            result = self._test_collective_on_shrunk_group(
    ...
    @requires_nccl()
    @skip_but_pass_in_sandcastle_if(
        int(os.environ.get("WORLD_SIZE", "1")) < 4, "Test requires world size >= 4"
    )
    @skip_if_lt_x_gpu(int(os.environ.get("WORLD_SIZE", "1")))
    def test_shrink_group_multiple_exclusions(self):
        ...
        ranks_to_exclude = [
            i for i in range(2, self.world_size, 2)
        ]  # Every other rank from 2

        self._perform_shrink_test(ranks_to_exclude, "Multiple exclusions test")

    @requires_nccl()
    @skip_but_pass_in_sandcastle_if(
        int(os.environ.get("WORLD_SIZE", "1")) < 3, "Test requires world size >= 3"
    )
    @skip_if_lt_x_gpu(int(os.environ.get("WORLD_SIZE", "1")))
