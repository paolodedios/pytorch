...
class FakeTensorViewCopy(TestCase):
    ...
    def test_mkldnn_view_errors_match_eager(self):
        ...
        with FakeTensorMode() as fake_mode:
            ...
            for output in (fake_x.detach(), torch.relu(fake_x), fake_x.clone()):
            ...
            for clone in (
                ...
                with self.assertRaisesRegex(RuntimeError, "Change to use reshape"):
                    output.view(output.size(0), -1)
            with self.assertRaisesRegex(RuntimeError, "unsupported memory format"):
    ...
    @unittest.skipIf(not torch.backends.mkldnn.is_available(), "requires MKLDNN")
    def test_mkldnn_flatten_matches_eager(self):
        x = torch.randn(1, 512, 1, 1, dtype=torch.float32).to_mkldnn()
        expected_shape = torch.flatten(x, 1).shape

        with FakeTensorMode() as fake_mode:
            fake_x = fake_mode.from_tensor(x)

            for output in (
                torch.flatten(fake_x, 1),
                fake_x.flatten(1),
                torch.flatten(input=fake_x, start_dim=1),
                torch.ops.aten.flatten.using_ints(fake_x, 1, -1),
            ):
            ...
            self.assertIs(torch.flatten(fake_x, 2, 2), fake_x)
            with self.assertRaisesRegex(
                RuntimeError, "start_dim cannot come after end_dim"
            ):
                torch.flatten(fake_x, 2, 1)

        def flatten_case(x):
            return torch.flatten(x, 1)

        torch._dynamo.reset()
        output = torch.compile(flatten_case, backend="eager", fullgraph=True)(x)
        self.assertEqual(output.shape, expected_shape)
        self.assertTrue(output.is_mkldnn)
    ...
    def test_mkldnn_max_pool_with_indices_errors_match_eager(self):
        ...
        with FakeTensorMode() as fake_mode:
            ...
            for max_pool2d in (
                ...
                with self.assertRaisesRegex(
                    RuntimeError, "stride should be greater than zero"
                ):
                    max_pool2d()
            with self.assertRaisesRegex(
                RuntimeError, "stride should be greater than zero"
            ...
            with self.assertRaisesRegex(RuntimeError, "stride should not be zero"):
                torch.ops.aten.max_pool2d_with_indices.default(
                    fake_x2, [3, 3], [0, 0], [1, 1], [1, 1], False
                )
            with self.assertRaisesRegex(
