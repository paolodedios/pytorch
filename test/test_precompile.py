...
class TestPrecompile(TestCase):
    ...
    def test_cache_envelope_is_weights_only_safe(self):
        ...
        # (weights_only=True). The executable part is the inner artifact bytes,
        # reconstructed by CompiledArtifact.load inside load() -- that is the only
        # code-execution step, not this outer load. The integrity tag is present and
        # correct (and itself weights_only-safe).
        from torch._precompile import _CACHE_FORMAT, _CACHE_VERSION
    ...
    def test_untrusted_input_warning_fires_per_load(self):
        # The trust warning is emitted PER load (not warning_once), on BOTH the
        # cache-reconstruction path and the inlined-exec path, via log.warning on the
        # torch._precompile logger (load() warns; _make_inlined_forward warns again before
        # exec). Calling load() TWICE must fire the untrusted-input warning on BOTH calls,
        # locking in per-load behavior rather than once-per-process.
        m = torch.nn.Sequential(torch.nn.Linear(4, 3)).eval()
        x = torch.randn(5, 4)
        # Cached path (inductor): load() warns about treating both inputs as untrusted.
        ...
        # Inlined-exec path (eager backend has an empty cache, so load() EXECs
        # python_code): both load() and _make_inlined_forward warn, on every load.
        ecode, ecache = torch.compiler.precompile(
    ...
    def test_single_trust_warning_on_inlined_load(self):
        # On the inlined load path (an eager artifact has an empty cache, so load() EXECs
        # python_code) the untrusted-input / EXEC warning must fire EXACTLY ONCE -- only
        # _make_inlined_forward warns; the cached-reconstruction warning is not emitted
        # because there is no artifact to reconstruct. Asserting "exactly once" guards
        # against the warning being duplicated across both paths on this load.
        m = torch.nn.Sequential(torch.nn.Linear(4, 3)).eval()
        x = torch.randn(5, 4)
        code, cache = torch.compiler.precompile(
            lambda model, t: model(t), m, x, backend="eager"
