...
def forward(self, p_linear_weight, p_linear_bias, b_buffer, x):
    ...
    def test_unbacked_infer_size_nonzero_broadcast(self):
        ...
        x3 = torch.zeros(3, 2)
        y3 = torch.tensor([[0, 0], [6, 0], [0, 0]], dtype=torch.float32)
        dynamic_shapes = {
            "x": {0: Dim.DYNAMIC, 1: Dim.DYNAMIC},
        ...
        for model in (AddFoo(), ExpandFoo()):
            ...
            self.assertEqual(ep.module()(x3, y3), model(x3, y3))
...
def forward(self, x, y):
    ...
    def test_unbacked_expand(self):
        ...
        self.assertEqual(
            ep.module()(torch.tensor([1, 0])).shape[0],
            0,
        )
        with torch.fx.experimental._config.patch(backed_size_oblivious=True):
            ep_bso = export(Bar(), (torch.tensor([1, 1]),))
            self.assertEqual(
                ep_bso.module()(torch.tensor([1, 5])).shape[0],
                5,
            )
        with self.assertRaisesRegex(
            RuntimeError,
