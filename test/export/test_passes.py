...
def forward(self, token, obj_attr, x):
    with_effects = torch.ops.higher_order.effects.with_effects(token, torch.ops._TorchScriptTesting.takes_foo_tuple_return.default, foo = obj_attr, x = x);  token = x = None
    ...
    with_effects_1 = torch.ops.higher_order.effects.with_effects(getitem, torch.ops._TorchScriptTesting.takes_foo.default, foo = obj_attr, x = add);  getitem = obj_attr = add = None
