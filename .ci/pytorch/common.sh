...
if [[ -n "${ANACONDA_PYTHON_VERSION:-}" ]]; then
  ...
  if [[ -x "${CONDA_ENV_PYTHON}" ]]; then
    ...
    # env?" — it's pinned in that file but is not a default conda dep.
    if [[ -f /opt/conda/requirements-ci.txt ]] && \
       ! "${CONDA_ENV_PYTHON}" -c 'import build' >/dev/null 2>&1; then
      echo "Re-installing /opt/conda/requirements-ci.txt into ${CONDA_ENV_PREFIX}"
      "${CONDA_ENV_PYTHON}" -m pip install -r /opt/conda/requirements-ci.txt
    fi
  fi
