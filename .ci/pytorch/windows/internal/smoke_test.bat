...
set PYTHON_EXECUTABLE="python-amd64.exe"
if "%DESIRED_PYTHON%" == "3.13t" (
    set PYTHON_EXECUTABLE="freethreaded.msi"
)

del %PYTHON_EXECUTABLE%
curl --retry 3 -kL "%PYTHON_INSTALLER_URL%" --output %PYTHON_EXECUTABLE%
if errorlevel 1 exit /b 1
...
start /wait "" %PYTHON_EXECUTABLE% /quiet InstallAllUsers=1 PrependPath=0 Include_test=0 TargetDir=%CD%\Python
if errorlevel 1 exit /b 1
