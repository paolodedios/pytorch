...
bool ProcessGroupNCCL::dumpDebuggingInfo(bool captureStackTrace) {
  ...
  LOG(ERROR)
      << logPrefix()
      << "ProcessGroupNCCL preparing to dump debug info. Capture stack trace: "
      << captureStackTrace;
  if (traceBufferSize_ > 0) {
    // We dump nccl trace into local disk by default and users can register
    // their customized writer by inheriting `DebugInfoWriter` via
    // `registerDebugInfoWriter`.
    auto ncclTrace = dump_nccl_trace(true, captureStackTrace, false);
...
void ProcessGroupNCCL::heartbeatMonitor() {
  ...
  if (checkDumpSignal && shouldDump_.load()) {
    ...
    // local disk)
    std::future<bool> asyncDebugDumpWithStack = std::async(
        std::launch::async, [this]() { return this->dumpDebuggingInfo(); });

    // wait for the dump until timeout - log data
    auto result = waitForFutureOrTimeout(
        asyncDebugDumpWithStack,
        std::chrono::milliseconds(waitTimeoutDumpInMilSec_),
        "Flight recorder dump in heartbeatMonitor",
        false,
        true);

    // If we failed to dump, try dumping without stack trace.
    if (!result) {
      std::future<bool> asyncDebugDumpWithNoStack =
          std::async(std::launch::async, [this]() {
            return this->dumpDebuggingInfo(/*captureStackTrace=*/false);
          });
      waitForFutureOrTimeout(
          asyncDebugDumpWithNoStack,
          std::chrono::milliseconds(waitTimeoutDumpInMilSec_),
          "Flight recorder dump in heartbeatMonitor",
          false,
          true);
