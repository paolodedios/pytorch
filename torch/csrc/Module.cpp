...
#define ASSERT_TRUE(cmd) \
...

  {
    ...
    py_module.def(
        "_fake_dispatch_register_decomp", add_for(FakeDispatchCategory::Decomp));
    py_module.def(
        "_fake_dispatch_register_meta", add_for(FakeDispatchCategory::Meta));
    py_module.def(
        "_fake_dispatch_register_op_impl", add_for(FakeDispatchCategory::OpImpl));
    py_module.def(
    ...
    py_module.def(
        "_fake_dispatch_has_decomp",
        [](const std::string& name, const std::string& overload) {
          return at::impl::fakeHasUsableDecomp(
              c10::OperatorName(name, overload));
        });
    py_module.def(
        "_fake_dispatch_has_prim_meta",
        [](const std::string& name, const std::string& overload) {
          return at::impl::fakeHasPrimMeta(c10::OperatorName(name, overload));
        });
    py_module.def(
        "_fake_dispatch_has_op_impl",
        [](const std::string& name, const std::string& overload) {
          return at::impl::fakeHasOpImpl(c10::OperatorName(name, overload));
        });
  }
