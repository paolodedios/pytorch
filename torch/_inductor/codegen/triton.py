...
class TensorDescriptorOptions(BlockDescriptorOptions):
    ...
    def create(
        ...
        *,
        ...
        """
        TMA's implementation is equivalent, but it defaults
        transpose_contiguous to True (which is not implemented)
        for regular BlockPtrOptions yet.

        Due to possible performance considerations, TMA can still
...
class TritonKernel(SIMDKernel[TritonCSEVariable]):
    ...
    def load(self, name: str, index: sympy.Expr):
        ...
        indexing = self.indexing(
            ...
            tma_compatibility_checker=self.tma_compatibility_checker_cls(
                self, dtype, for_store=False, force=False,
            ),
        )
    ...
    def store(
        ...
        if mode is None or mode == "tma":
            force = mode == "tma"
            tma_compatibility_checker = self.tma_compatibility_checker_cls(
                self, dtype, for_store=True, force=force,
            )
        indexing = self.indexing(
            index,
            dense_indexing=True,
    ...
    def store_reduction(
        ...
        indexing = self.indexing(
            ...
            block_ptr=True,
            tma_compatibility_checker=self.tma_compatibility_checker_cls(
                kernel=self, dtype=dtype, for_store=True, force=False,
            ),
        )
