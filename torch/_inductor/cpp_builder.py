...
def get_cpp_options(
    ...
    if not is_target_windows() and config.aot_inductor.enable_lto and _is_clang(cpp_compiler):
...
def _get_torch_related_args(
    ...
    # TODO: why commenting this out?
    # TODO: not include pytorch?
    # if is_target_windows():
...
class CppBuilder:
    ...
    def save_compile_cmd_to_cmake(
        ...
        # TODO: add flag to remove libtorch dependency

        target_library_type = (
            "STATIC" if config.aot_inductor.compile_standalone and config.aot_inductor.cross_target_platform is None  else "SHARED"
        )
