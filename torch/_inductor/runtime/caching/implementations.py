...
class _CacheImpl(ABC):
    ...
    @property
    def lock(self: Self) -> Callable[[float], Generator[None, None, None]]:
        ...
        def _lock_with_timeout(timeout: Optional[float] = None) -> Generator[None, None, None]:
            return locks._acquire_lock_with_timeout(self._lock, timeout)
        return _lock_with_timeout

    @abstractmethod
    def get(self: Self, key: Any) -> Optional[Hit]:
    ...
    @abstractmethod
    def insert(self: Self, key: Any, value: Any) -> bool:
...
class _InMemoryCacheImpl(_CacheImpl):
    ...
    def get(self: Self, key: Any) -> Optional[Hit]:
        ...
        if (value := self._memory.get(pickled_key, "miss")) != "miss":
...
class _OnDiskCacheImpl(_CacheImpl):
    ...
    def _base_dir(self: Self) -> Path:
        ...
        return Path(default_cache_dir(), "cache", self._sub_dir)
...
try:
    ...
    class _RemoteCacheImpl(_CacheImpl):  # type: ignore[no-redef]
        ...
        def lock(self: Self) -> Callable[[float], Generator[None, None, None]]:
            ...
            @contextmanager
            def pseudo_lock(timeout: Optional[float] = None) -> Generator[None, None, None]:
                yield
            return pseudo_lock
