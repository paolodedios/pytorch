...
class _CacheIntf(ABC):    
    ...
    def _make_key(
        ...
        fkey: Any = (callee, params) if not custom_params_encoder else (callee, custom_params_encoder(*params[0], **params[1]))
    ...
    def insert(
        ...
        _intf_callback(
            ...
            _IntfCallbackAction.INSERTED if inserted else _IntfCallbackAction.NOT_INSERTED,
            dur,
            fn,
            params,
            result,
...
class _FastCacheIntf(_CacheIntf):
    ...
    def _get_odc_from_callee(self, callee: str) -> impls._OnDiskCacheImpl:
        if not (odc := self._callee_to_odc.get(callee)):
            callee_sub_dir: PathLike[str] = callee
        ...
        def wrapper(*args: P.args, **kwargs: P.kwargs) -> R:
            ...
            with intf.lock():
                ...
                if get:
                    ...
                    _intf_callback(
                        _IntfCallbackOrigin.RECORD,
                        _IntfCallbackAction.RECORD_INSERTED if inserted else _IntfCallbackAction.RECORD_NOT_INSERTED,
                        dur,
                        fn,
                        params,
                        result,
...
class _DeterministicCacheIntf(_CacheIntf):
    def __init__(self) -> None:
        ...
        if config.STRICTLY_PRE_POPULATED_DETERMINISM:
            self._get_sc_from_callee: Callable[[str], None | impls._OnDiskCacheImpl | impls._RemoteCacheImpl] = lambda callee: None
                ...
                try:
                    ...
                    for key, value in existing_dump.items():
                        if key not in to_dump:
                            ...
                            raise exceptions.DeterministicCachingIMCDumpConflictError
