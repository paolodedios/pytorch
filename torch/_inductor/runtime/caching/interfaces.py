...
class DeferredRecording(Generic[_R, _EncodedR]):
    ...
    def finalize(self, encoded_result: _EncodedR) -> None:
        ...
        """
        callbacks_to_invoke: list[Callable[[_EncodedR], None]] = []
        with self._lock:
            ...
            # Take ownership of callbacks and mark as completed
            callbacks_to_invoke = self._callbacks
            self._callbacks = None
            self._encoded_result = encoded_result

        # Execute callbacks outside the lock to prevent potential deadlocks
    ...
    def register_callback(self, callback: Callable[[_EncodedR], None]) -> None:
        ...
        """
        should_invoke_immediately = False
        with self._lock:
            if self._callbacks is None:
                should_invoke_immediately = True
            else:
                self._callbacks.append(callback)

        # Execute callback outside the lock to prevent potential deadlocks
...
class ResultEncoderFactory(Protocol[_P, _R, _EncodedR]):
    """Protocol for result encoder factory factories.

    A result encoder factory factory is a callable that:
    1. First takes the underlying unwrapped function `fn`
    2. Returns a factory that takes the same arguments as the memoized function
    3. That factory returns an encoder function that converts R -> _EncodedR
       (or DeferredRecording)
    ...
    Example:
        def my_encoder_factory(fn):
            def params_to_encoder(*args, **kwargs):
                def encode(result):
...
class ResultDecoderFactory(Protocol[_P, _R, _EncodedR]):
    """Protocol for result decoder factory factories.

    A result decoder factory factory is a callable that:
    1. First takes the underlying unwrapped function `fn`
    2. Returns a factory that takes the same arguments as the memoized function
    3. That factory returns a decoder function that converts _EncodedR -> R
    ...
    Example:
        def my_decoder_factory(fn):
            def params_to_decoder(*args, **kwargs):
    ...
    def record(
        ...
        Args:
            custom_params_encoder: Optional encoder for function parameters.
                                  If None, parameters are pickled directly.
            custom_result_encoder: Optional encoder factory for function results.
                                  Takes function parameters plus an optional `fn` keyword
                                  argument (the underlying unwrapped function), and returns
                                  an encoder function that converts R -> _EncodedR.
    ...
    def replay(
        ...
        Args:
            ...
            custom_result_decoder: Optional decoder factory for cached results.
                                  Takes function parameters plus an optional `fn` keyword
                                  argument (the underlying unwrapped function), and returns
                                  a decoder function that converts _EncodedR -> R.
