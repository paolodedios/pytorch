...
def _collect_runtime_context(
    ...
    if selection.get("inductor_configs"):
        result["inductor_configs"] = _RuntimeContext.inductor_configs()
    if selection.get("torch_determinism_configs"):
        result["torch_determinism_configs"] = (
            _RuntimeContext.torch_determinism_configs()
        )
    if selection.get("cuda_matmul_precision_configs"):
        result["cuda_matmul_precision_configs"] = (
            _RuntimeContext.cuda_matmul_precision_configs()
        )
...
def _collect_compile_context(
    ...
    result: dict[str, object] = {}
    if selection.get("torch_version_hash"):
        result["torch_version_hash"] = _CompileContext.torch_version_hash()
    if selection.get("triton_version_hash"):
        result["triton_version_hash"] = _CompileContext.triton_version_hash()
    if selection.get("runtime"):
        result["runtime"] = _CompileContext.runtime()
    if selection.get("runtime_version"):
        result["runtime_version"] = _CompileContext.runtime_version()
    if selection.get("accelerator_properties"):
        result["accelerator_properties"] = _CompileContext.accelerator_properties()
    return result or None
