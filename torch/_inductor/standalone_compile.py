...
class CacheCompiledArtifact(CompiledArtifact):
    ...
    def _to_binary_bytes(self) -> bytes:
        ...
        Produces exactly the bytes that ``save(format="binary")`` writes to disk,
        so callers that only need the bytes (e.g. ``_binary_cache_bytes`` feeding
        ``compile_to_python``'s ``cache``) can avoid a temp file. The byte layout is
        the one ``load(format="binary")`` reads back: header, ``torch_key``, the
        autograd-cache ``key`` string, then the opaque ``artifact_bytes``.
        """
        if self._artifacts is None:
            raise RuntimeError(
    ...
    *,
    ...
    with _standalone_context(gm, dynamic_shapes, aot, fake_mode):
        ...
        if artifacts is None:
            # A None artifact is always benign: every caller falls back to the
            # python_code JIT path and degrades gracefully, so there is nothing to
            # surface to the user. It is also the EXPECTED outcome for several
            # legitimate cases -- caches intentionally disabled
            # (force_disable_caches/fx_graph_cache), and uncacheable graphs such as
            # control-flow HOPs (torch.cond/while_loop) whose lowering produces no
            # bundled artifact even with caches on. Log at debug only.
            log.debug(
                "standalone_compile produced no cache artifact; falling back to "
                "python_code JIT. This is expected when caches are disabled "
                "(force_disable_caches/fx_graph_cache) or for uncacheable graphs "
                "such as control-flow HOPs (torch.cond/while_loop)"
            )
