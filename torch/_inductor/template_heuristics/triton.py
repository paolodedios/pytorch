...
# Padding factor for shared memory estimator to account for
# small shared memory usage of kernels, e.g. barriers
SHARED_MEM_ESTIMATION_PADDING_FACTOR = 1.02
...
class BaseConfigHeuristic(metaclass=BaseHeuristicSingleton):
    ...
    def get_shared_memory_estimation(
        ...
        if has_sm_layout_conversion:
            shared_mem_epilogue = (
                layout_conversion_byte_size * gemm_config.block_m * gemm_config.block_n
            )
        else:
            shared_mem_epilogue = 0

        return shared_mem_loads * gemm_config.num_stages + shared_mem_epilogue
    ...
    def _get_exceeding_shared_memory_checker(
        ...
        def exceeds(gemm_config: BaseConfig, dtype_size: int) -> bool:
            ...
            return (estimation * SHARED_MEM_ESTIMATION_PADDING_FACTOR) > sm_available
