...
def _aot_stage2b_compile_forward_or_inference(
    ...
    *,
    ...
    - the output strides of the forward graph (None for inference mode)
    ...
    # Determine contexts and mode
    disable_amp = torch._C._is_any_autocast_enabled()
    if is_inference:
        autocast_context = torch._C._DisableAutocast if disable_amp else nullcontext
        outer_context = nullcontext()
    else:
        autocast_context = torch._C._DisableAutocast
        outer_context = torch.no_grad()

    tracking_mode = "inference" if is_inference else "forward"

    with outer_context:
        with autocast_context(), track_graph_compiling(aot_config, tracking_mode):
            # Setup wrappers
            ...
            compiler = aot_config.inference_compiler if is_inference else aot_config.fw_compiler
