...
def _are_blocks_contiguous(block_mask: "BlockMask") -> bool:
    ...
    Checks both KV-side and Q-side indices (partial and full).
    """
    ...
    if block_mask.q_indices is not None and block_mask.q_num_blocks is not None:
        if not _check_indices(block_mask.q_indices, block_mask.q_num_blocks):
            return False
    if (
        ...
        if not _check_indices(
            block_mask.full_q_indices, block_mask.full_q_num_blocks
        ):
            return False
    return True
