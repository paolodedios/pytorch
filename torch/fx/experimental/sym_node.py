...
def _optimized_add(
    ...
    lhs_is_optimized_summation = (
        lhs_is_optimized_summation or _is_symbols_binary_summation(lhs)
    )
    rhs_is_optimized_summation = (
        rhs_is_optimized_summation or _is_symbols_binary_summation(rhs)
    )

    # (a0+a1) + (a2+a3) => (a0+a1+a2+a3)
    if lhs_is_optimized_summation and rhs_is_optimized_summation and sortkey(lhs._args[-1]) < sortkey(
        rhs._args[0]
    ):
