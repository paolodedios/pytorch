...
def _merge_connected_partitions(
    ...
    """Merge adjacent partitions that are connected by data dependencies.

    When an untagged node slips between two tagged runs that share a data edge,
    keeping them separate forces an unnecessary materialisation at the partition
    boundary. This pass merges such adjacent partitions while leaving unrelated
    partitions untouched.

    Only partitions with the same compile_with_inductor annotation are merged,
    so distinct compilation regions (potentially with different inductor configs)
    are never fused together.
    """
    if len(partitions) <= 1:
        return partitions
