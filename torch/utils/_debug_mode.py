...
_DISPATCH_PRE_HOOKS: list[Callable] = []
...
def _run_dispatch_pre_hooks(call: _DebugCall, func, types, args, kwargs) -> None:
    global _DISPATCH_PRE_HOOKS
    if _DISPATCH_PRE_HOOKS:
        for hook in _DISPATCH_PRE_HOOKS:
...
class DebugMode(TorchDispatchMode):
    ...
    def __torch_dispatch__(self, func, types, args=(), kwargs=None):
        ...
        # in-place mutation
        if call:
            _run_dispatch_pre_hooks(call, func, types, args, kwargs)
    ...
    def dispatch_hooks(
        ...
        pre_hook signature is (func, types, args, kwargs, call) and is executed before
        the operation. It allows capturing state before in-place mutations.
        """
        global _DISPATCH_RECORD_HOOKS, _DISPATCH_LOG_HOOKS, _DISPATCH_PRE_HOOKS
        ...
        if pre_hook:
            _DISPATCH_PRE_HOOKS.append(pre_hook)
        try:
            yield
        finally:
            if record_hook:
    ...
    def log_tensor_hashes(
        ...
        def _dispatch_pre_hook(func, types, args, kwargs, call):
        ...
        try:
            ...
            with DebugMode.dispatch_hooks(
                ...
                pre_hook=_dispatch_pre_hook if hash_inputs else None,
            ):
                yield
        finally:
            if hash_inputs:
