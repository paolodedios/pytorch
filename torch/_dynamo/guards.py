        ...
        if match_on_id_for_tensor(guard):
            ...
            if self.check_fn_manager.output_graph.export:
                ...
                if not isinstance(value, torch.nn.Parameter):
                    self.guard_manager.diff_guard_sources.add(
                        guard.name
                    )
...
class CheckFunctionManager:
    def __init__(
        ...
        if guard_filter_fn:
            ...
            filter_results = guard_filter_fn([make_guard_filter_entry(guard) for guard in sorted_guards])
            assert len(filter_results) == len(sorted_guards)
            assert all(type(x) == bool for x in filter_results)
            sorted_guards = [guard for i, guard in enumerate(sorted_guards) if filter_results[i]]
            # Redo the guards because filtering relies on the results from the last guard builder.
            builder, guard_manager = self.build_guards(sorted_guards, existing_diff_guard_sources, f_code, output_graph)
