...
class ConstDictVariable(VariableTracker):
    ...
    def has_new_items(self) -> bool:
        return self.should_reconstruct_all or any(
            self.is_new_item(self._get_original_item(key), value)
            for key, value in self.items.items()
        )
    ...
    def _items_match_original(self) -> bool:
        if len(self.items) != len(self.original_items):
            return False
        original_keys = [
            key if isinstance(key, HashableTracker) else HashableTracker(key)
            for key in self.original_items
        ]
        if list(self.items.keys()) != original_keys:
            return False
        return all(
            (original := self._get_original_item(key)) is not None
            and not self.is_new_item(original, value)
            for key, value in self.items.items()
        )

    def _get_original_item(self, key: HashableTracker) -> VariableTracker | None:
        original = self.original_items.get(key.vt)
        if original is None:
            original = self.original_items.get(key)  # type: ignore[arg-type]
        return original
    ...
    def reconstruct_kvs_into_new_dict(self, codegen: "PyCodegen") -> None:
        ...
        for key, value in self.items.items():
            ...
            item = self._get_original_item(key)
        ...
        if name == "__init__":
            ...
            if args:
                ...
                if isinstance(other, ConstDictVariable):
                    ...
                    self._check_removable_handle_id_dict_merge(tx, other)
                    ...
                    for key in unpack_iterable(tx, keys):
                        if self._has_removable_handle_id_key() and not (
                            _is_removable_handle_id_key(key)
                        ):
                            _graph_break_removable_handle_id(tx)
                    ...
                    for idx, item in enumerate(unpack_iterable(tx, other)):
                        ...
                        if self._has_removable_handle_id_key() and not (
                            _is_removable_handle_id_key(pair[0])
                        ):
                            _graph_break_removable_handle_id(tx)
                        self._check_removable_handle_id_insert(tx, pair[0])
