...
def get_triton_kernel_and_cache_entry(node: torch.fx.Node):
    assert node.target is torch._higher_order_ops.triton_kernel_wrap.triton_kernel_wrapper_functional
    ...
    if hasattr(kernel, "device_caches"):
        ...
        assert False, "kernel caches not found"
    ...
    if len(cache.keys()) == 1:
        ...
        assert False, (
            "couldn't find a kernel cache entry with metadata matching the autotuner configs"
        )
