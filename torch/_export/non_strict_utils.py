    ...
    for input_index, node in enumerate(gm.graph.nodes):
        ...
        for i, d in enumerate(node.meta["val"].shape):
            ...
            if isinstance(dim, _DimHint):
                ...
                except torch.utils._sympy.value_ranges.ValueRangeError as exc:
                    raise ValueError(
                        f"Received user-specified min/max range of [{dim.min}, {dim.max}], "
                        f"which conflicted with the inferred min/max range of [{trace_vr.lower}, {trace_vr.upper}] "
                        f"from model tracing, for inputs{pytree.keystr(flat_paths[input_index])}.shape[{i}]."
                    ) from exc
