...
REGISTER_CPU_KERNEL(
    ...
    aten__scaled_mm_default,
    {
      const auto& self = KernelInput(0).toTensor();
      const auto& mat2 = KernelInput(1).toTensor();
      const auto& scale_a = KernelInput(2).toTensor();
      const auto& scale_b = KernelInput(3).toTensor();
      const auto bias = KernelInput(4).toOptional<at::Tensor>();
      const auto scale_result = KernelInput(5).toOptional<at::Tensor>();
      const auto out_dtype = KernelInput(6).toOptional<at::ScalarType>();
      const auto use_fast_accum = KernelInput(7).toBool();
      if (KernelOutput(0).isNone()) {
        KernelOutput(0) = at::native::_scaled_mm_cpu(
            self,
            mat2,
            scale_a,
            scale_b,
