...
Weights::Weights(
  ...
  auto loadAndInsert = [&](const std::string& tensorName,
    ...
    TORCH_CHECK(
        ...
        tensorName,
        " in tensorPaths");
    const std::string tensorPathBase = pathIt->second;
    const std::string tensorPath = std::string{pathPrefix} + tensorPathBase;
    ...
    if (weightsMeta_.find(tensorName) != weightsMeta_.end()) {
      ...
      TORCH_CHECK(
          false, "Tensor meta not found for: ", tensorName, " in base weights.")
    }

    std::optional<TensorMeta> newTensorMeta;
    if (maybeNewWeightsMeta) {
      if (maybeNewWeightsMeta->find(tensorPathBase) !=
          maybeNewWeightsMeta->end()) {
        newTensorMeta = *maybeNewWeightsMeta->at(tensorPathBase);
      } else if (maybeNewWeightsMeta->find(tensorName) !=
                 maybeNewWeightsMeta->end()) {
        newTensorMeta = *maybeNewWeightsMeta->at(tensorName);
      } else {
        TORCH_CHECK(
            false,
