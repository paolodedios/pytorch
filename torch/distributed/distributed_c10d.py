...
# Define constants for NCCL shrink flags if not already defined elsewhere
# (These might ideally live in a more central place like torch.distributed.constants)
NCCL_SHRINK_DEFAULT = 0x00
NCCL_SHRINK_ABORT = 0x01
NCCL_MIN_VERSION_FOR_SHRINK = (2, 27, 0)
...
def shrink_group(
    ...
    """Shrinks a ProcessGroupNCCL instance by excluding specified ranks.

    Creates and returns a new, smaller ``ProcessGroupNCCL`` instance comprising
    only the ranks from the original group that were *not* in the
    ``ranks_to_exclude`` list.

    This function requires the backend to be NCCL and the underlying NCCL version
    must support the ``ncclCommShrink`` API.

    - **ONLY non-excluded ranks** should call this function
    ...
    Args:
        ...
        shrink_flags (int, optional): Flags to control the shrinking behavior.
            Can be ``NCCL_SHRINK_DEFAULT`` (default) or ``NCCL_SHRINK_ABORT``.
            ``NCCL_SHRINK_ABORT`` will attempt to terminate ongoing operations
            in the parent communicator before shrinking.
            Defaults to ``NCCL_SHRINK_DEFAULT``.

    Returns:
        ProcessGroup: Returns a new ``ProcessGroupNCCL`` instance representing the
            shrunk group. The returned group becomes the new default group for
            default group shrinks.

    Raises:
        TypeError: If the provided ``group`` or the default group is not using NCCL backend.
        RuntimeError: If the NCCL version does not support ``ncclCommShrink``,
                      if an excluded rank calls this function, or if any other NCCL error occurs.
        ValueError: If ranks_to_exclude is invalid (empty, out of bounds, duplicates, or excludes all ranks).

    .. note:: **Usage patterns:**

              **For non-excluded ranks:**
              ```python
              # This rank participates in shrink
              new_pg = shrink_group([failed_rank])
              # Continue training with new_pg as default
              ```

              **For excluded ranks:**
              ```python
              # In real failure scenarios: rank process terminates
              ```

    .. warning:: **Fault tolerance considerations:**

                  1. **ONLY non-excluded ranks call shrink_group()**
                  2. **Excluded ranks should terminate their process** (resources auto-cleaned by OS)
                  3. **No resource leaks** - process termination provides complete cleanup

                  **Key Design**: Non-excluded ranks continue training; excluded ranks terminate cleanly.

                  When shrinking the default group, all other process groups are automatically
                  destroyed because they become invalid when global ranks are reassigned by NCCL.
    """
    ...
    # Step 3: Validate NCCL backend requirements and availability
    nccl_backend = _validate_nccl_shrink_requirements(target_group_info)
    ...
    # Step 5: Execute the actual NCCL shrink operation
    new_nccl_backend = nccl_backend.shrink(sorted(excluded_ranks_set), shrink_flags)

    # Step 6: Handle cleanup and creation of new process group
    return _finalize_shrunk_group(
        target_group_info, excluded_ranks_set, new_nccl_backend
    )


def _validate_shrink_inputs(ranks_to_exclude: list[int], shrink_flags: int) -> None:
    """Validate input parameters for shrink_group function."""
    ...
    valid_flags = [NCCL_SHRINK_DEFAULT, NCCL_SHRINK_ABORT]
    if not isinstance(shrink_flags, int) or shrink_flags not in valid_flags:
        raise ValueError(
            ...
            f"NCCL_SHRINK_DEFAULT ({NCCL_SHRINK_DEFAULT}) or NCCL_SHRINK_ABORT ({NCCL_SHRINK_ABORT}). "
            f"Use NCCL_SHRINK_ABORT to abort ongoing operations before shrinking."
        )
...
def _prepare_shrink_target_group(group: Optional[ProcessGroup]) -> dict:
    ...
    if group_info["group_size"] <= 1:
        raise ValueError(
            f"Cannot shrink a process group with size {group_info['group_size']}. "
...
def _validate_nccl_shrink_requirements(group_info: dict) -> "ProcessGroupNCCL":
    """Validate NCCL backend availability and version requirements."""
    # Check NCCL availability first
    if not is_nccl_available():
        raise RuntimeError(
            "Cannot shrink process group: NCCL backend is not available. "
            "Ensure PyTorch was compiled with NCCL support and NCCL libraries are installed."
        )

    # Validate NCCL version supports shrinking
    nccl_version = torch.cuda.nccl.version()
    if nccl_version < NCCL_MIN_VERSION_FOR_SHRINK:
        raise RuntimeError(
            f"ProcessGroupNCCL.shrink requires NCCL version >= {NCCL_MIN_VERSION_FOR_SHRINK}. "
            f"Found NCCL version {nccl_version}. Please update NCCL to use shrinking functionality."
        )

    # Import ProcessGroupNCCL for type checking
    try:
        from torch._C._distributed_c10d import ProcessGroupNCCL
    except ImportError as e:
        raise RuntimeError(
            "ProcessGroupNCCL type not found. NCCL backend might not be available or properly initialized. "
            "Ensure CUDA and NCCL are properly configured."
        ) from e

    # Get and validate the NCCL backend from the target group
    target_pg = group_info["process_group"]
    try:
        nccl_backend = target_pg._get_backend(torch.device("cuda"))
    except RuntimeError as e:
        raise RuntimeError(
            f"Cannot access CUDA backend for process group '{group_info['group_name']}'. "
            f"Ensure the process group was initialized with NCCL backend and CUDA devices are available."
        ) from e

    if not isinstance(nccl_backend, ProcessGroupNCCL):
        raise TypeError(
            f"Expected ProcessGroupNCCL backend for group '{group_info['group_name']}', "
            f"but got {type(nccl_backend).__name__}. Only NCCL process groups support shrinking operations."
        )
...
def _finalize_shrunk_group(
    group_info: dict, excluded_ranks_set: set, new_nccl_backend
) -> ProcessGroup:
    ...
    new_pg = _create_shrunk_process_group(
        new_nccl_backend, remaining_ranks, original_group_metadata, is_default_group
    )
...
def _extract_group_metadata(target_pg: ProcessGroup) -> dict:
    ...
    remaining_ranks = [rank for rank in get_process_group_ranks(target_pg)]
    new_group_name = _process_group_name(remaining_ranks, use_hashed_name=False)
...
def _cleanup_original_group(target_pg: ProcessGroup, is_default_group: bool) -> None:
    ...
    try:
        ...
        logger.warning(f"Failed to destroy {group_type} group during shrinking: {e}")
...
def _create_shrunk_process_group(
    new_nccl_backend, remaining_ranks: list[int], metadata: dict, is_default_group: bool
) -> ProcessGroup:
    """Create and configure the new shrunk process group."""
    # Create new group properties
    new_group_rank = new_nccl_backend.rank()
    new_group_size = new_nccl_backend.size()
    ...
    # Create process group with new communicator
    prefix_store = PrefixStore(f"{group_name}/", metadata["store"])
    new_pg = ProcessGroup(prefix_store, new_group_rank, new_group_size)

    # Configure NCCL backend
    backend_type = ProcessGroup.BackendType.NCCL
    new_pg._register_backend(torch.device("cuda"), backend_type, new_nccl_backend)
...
def _destroy_all_other_groups(exclude_group: Optional[ProcessGroup] = None) -> None:
    ...
    if groups_to_destroy:
        ...
        logger.warning(
            f"Shrinking default group will destroy {len(groups_to_destroy)} other process groups: "
            f"{', '.join(group_names)}. This is necessary because shrinking the default group "
            f"reassigns global ranks, making existing groups inconsistent."
        )
    ...
    for pg in groups_to_destroy:
        try:
            ...
            logger.warning(
                f"Failed to abort process group {_get_process_group_name(pg)}: {e}"
            )
...
def _cleanup_process_group_global_state(pg: ProcessGroup) -> None:
    ...
    try:
        ...
        logger.warning(f"Failed to fully clean up global state for process group: {e}")
