...
def _parse_backend_string(
    ...
    Only device types in *available_devices* are included in the result.
    This prevents bare backend names like ``"gloo"`` from expanding to
    device types not present in the caller's context (e.g. ``"mps"`` on a
    CUDA-only system).
    ...
    if ":" in backend:
        ...
        device_backends = {
            d: b for d, b in device_backends.items() if d in available_devices
        }
