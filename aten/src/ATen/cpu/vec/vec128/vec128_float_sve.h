...
inline float32x4_t vlogq_f32(float32x4_t x) const {
    ...
    // Extract exponent
    int32x4_t   m   = svget_neonq(svsub_n_s32_x(ptrue, svset_neonq(svundef_s32(), vreinterpretq_s32_u32(vshrq_n_u32(vreinterpretq_u32_f32(x), 23))), 127));
  ...
  bool has_inf_nan() const {
    return svptest_any(
        ptrue, svcmpuo_f32(ptrue, svset_neonq(svundef_f32(), vsubq_f32(values, values)), ZERO_F32));
  }
...
#define DEFINE_SLEEF_COMPATIBLE_BINARY_ELEMENTWISE_FUNC(name)      \
  ...
  Vectorized<float> tanh() const {
    ...
    float32x4_t x = vminq_f32(vmaxq_f32(values, CONST_MIN_TANH), CONST_MAX_TANH);

    // Step 2: Calculate exp(2 * x), where x is the clamped value.
    // svmul_f32_z computes 2 * x, and vexpq_f32 computes the exponential of
    // the result.
    svfloat32_t exp2x = svset_neonq(svundef_f32(), vexpq_f32(vmulq_f32(CONST_2, x)));
  ...
  Vectorized<float> rsqrt() const {
    return Vectorized<float>(vrsqrteq_f32(values));
  }
...
Vectorized<float> inline Vectorized<float>::eq(
  ...
  return svreinterpret_f32_u32(svand_n_u32_x(ptrue, svreinterpret_u32_f32(*this == other), 1));
}
...
inline void convert(const bool* src, float* dst, int64_t n) {
  ...
  for (uint64_t iters = count / twoRegsElemCount; iters > 0; --iters) {
    ...
    auto vec2 = svget_neonq(svld1ub_u32(svptrue_b8(), srcPtr + oneRegElemCount));
    auto vec1Mask = vtstq_u32(vec1, vec1);
    auto vec2Mask = vtstq_u32(vec2, vec2);
    vec1Mask = svget_neonq(svand_n_u32_x(svptrue_b8(), svset_neonq(svundef_u32(), vec1Mask), 0x3f800000));
    vec2Mask = svget_neonq(svand_n_u32_x(svptrue_b8(), svset_neonq(svundef_u32(), vec2Mask), 0x3f800000));
  ...
  if (remainder > 0) {
    ...
    auto vec1Mask = vtstq_u32(vec1, vec1);
    auto vec2Mask = vtstq_u32(vec2, vec2);
    vec1Mask = svget_neonq(svand_n_u32_x(pa, svset_neonq(svundef_u32(), vec1Mask), 0x3f800000));
    vec2Mask = svget_neonq(svand_n_u32_x(pb, svset_neonq(svundef_u32(), vec2Mask), 0x3f800000));
    svst1_f32(pa, dst, svset_neonq(svundef_f32(), vreinterpretq_f32_u32(vec1Mask)));
    svst1_f32(pb, dst + oneRegElemCount, svset_neonq(svundef_f32(), vreinterpretq_f32_u32(vec2Mask)));
  }
}
