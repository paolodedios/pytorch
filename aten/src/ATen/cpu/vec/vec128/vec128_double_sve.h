...
class Vectorized<double> {
 ...
 public:
  ...
  static Vectorized<double> arange(
    ...
    const Vectorized<double> step_sizes = svreinterpret_f64_u64(svindex_u64(0, 4607182418800017408)); // {0.0, 1.0}
  ...
  void store(void* ptr, int64_t count = size()) const {
    if (count == size()) {
      ...
      svbool_t pg = svwhilelt_b64(0ull, count);
      svst1_f64(pg, reinterpret_cast<double*>(ptr), svset_neonq(svundef_f64(), values));
    }
  }
  ...
  Vectorized<double> isnan() const {
    // NaN check
    return vreinterpretq_f64_u32(vmvnq_u32(vreinterpretq_u32_u64(vceqq_f64(values, values))));
  }
  bool has_inf_nan() const {
    return svptest_any(
        ptrue,
        svcmpuo_f64(ptrue, svset_neonq(svundef_f64(), vsubq_f64(values, values)), ZERO_F64));
  }
  ...
  Vectorized<double> angle() const {
    ...
    const auto nan_mask = svcmpuo_f64(ptrue, svset_neonq(svundef_f64(), values), ZERO_F64);
    const auto pi = svdup_n_f64(c10::pi<double>);

    const auto neg_mask = svcmplt_f64(ptrue, svset_neonq(svundef_f64(), values), ZERO_F64);
  ...
  Vectorized<double> operator!=(const Vectorized<double>& other) const {
    float64x2_t r0 =
        vreinterpretq_f64_u32(vmvnq_u32(vreinterpretq_u32_u64(vceqq_f64(values, other.values))));
...
Vectorized<double> inline Vectorized<double>::eq(
  ...
  return svreinterpret_f64_u64(svand_n_u64_x(ptrue, svreinterpret_u64_f64(*this == other), 1));
}
