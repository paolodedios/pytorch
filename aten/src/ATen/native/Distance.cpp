...
Tensor _pdist_forward(const Tensor& self, const double p) {
  ...
  TORCH_CHECK(device == kCPU || device == kCUDA || device == kXPU || device == kMPS,
      "_pdist_forward only supports CPU, XPU, CUDA and MPS devices, got: ", device);
...
Tensor _pdist_backward(const Tensor& grad, const Tensor& self, const double p, const Tensor& pdist) {
  ...
  auto device = self.device().type();
  TORCH_CHECK(device == kCPU || device == kCUDA || device == kXPU || device == kMPS,
      "_pdist_backward only supports CPU, XPU, CUDA and MPS devices, got: ", device);
