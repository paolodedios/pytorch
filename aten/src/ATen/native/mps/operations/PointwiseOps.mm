...
static void addcmul_mps_kernel(TensorIteratorBase& iter, const Scalar& value) {
  lib.exec_ternary_kernel(iter, "addcmul", value, addc_alpha_type(iter.common_dtype()));
}

static void addcdiv_mps_kernel(TensorIteratorBase& iter, const Scalar& value) {
  lib.exec_ternary_kernel(iter, "addcdiv", value, addc_alpha_type(iter.common_dtype()));
}
