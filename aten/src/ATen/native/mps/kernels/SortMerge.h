// Merge-sort kernels for MPS. Two paths:
...
// includer controls them. SortRadix.h depends on sort_init/sort_compare
// declared here, so include this first.
#pragma once
...
template <typename T, ::metal::enable_if_t<::metal::is_same_v<T, bool>, bool> = true>
inline T sort_init(bool desc) {
  return !desc;
}

template <typename T,
          ::metal::enable_if_t<!::metal::is_same_v<T, bool> && ::metal::is_floating_point_v<T>, bool> = true>
inline T sort_init(bool desc) {
...
template <typename T,
          ::metal::enable_if_t<!::metal::is_same_v<T, bool> && !::metal::is_floating_point_v<T>, bool> = true>
inline T sort_init(bool desc) {
  return desc ? metal::numeric_limits<T>::lowest() : metal::numeric_limits<T>::max();
}
...
inline int merge_partition(Ptr A, Ptr B, int a_sz, int b_sz, int diag, bool desc) {
...
inline void merge_step(const threadgroup T* A,
...
template <typename T, ::metal::enable_if_t<sizeof(T) == 1 && !::metal::is_same_v<T, bool>, bool> = true>
inline T sort_shuffle_xor(T v, ushort delta) {
...
inline void bitonic_substage(thread T (&v)[TN], thread IdxT (&idx)[TN], uint lane, bool desc) {
#pragma unroll
  for (short i = 0; i < TN; ++i) {
    ...
    if (pi > i) {
      ...
      bool vi_first = bitonic_lt<STABLE>(vi, ii, vp, ip, /*i_am_low=*/false, desc);
...
inline void simd_bitonic_sort4(thread T (&v)[4], thread IdxT (&idx)[4], uint lane, bool desc) {
...
inline void block_merge_sort(threadgroup T* tv, threadgroup IdxT* ti, uint lid, bool desc) {
  static_assert(TN == 4 && TPTG >= 32, "block_merge_sort requires TN==4 and TPTG>=32");
  int base = lid * TN;
  thread T lv[TN];
  thread IdxT li[TN];
  for (int i = 0; i < TN; ++i) {
...
template <typename T, typename IdxT, short TPTG, short TN, bool STABLE>
kernel void mb_sort_block(const device T* inp [[buffer(0)]],
...
inline void mb_merge_body(const device T* vi,
  ...
  const int Ast = A0 + merge_partition(vi + A0, vi + A1, asz0, bsz0, diag_lo, desc);
  const int Aed = A0 + merge_partition(vi + A0, vi + A1, asz0, bsz0, diag_hi, desc);
  ...
  merge_step<T, InIdxT, TN>(
      tgv + ap, tgv + Asz + md - ap, tgi + ap, tgi + Asz + md - ap, Asz - ap, Bsz - md + ap, lv, li, desc);
...
#define INSTANTIATE_SORT_VARIANT(T, TPTG, TN, STABLE, SUFFIX)                                                  \
  template[[host_name("sort_block_" #T "_tptg" #TPTG SUFFIX)]] kernel void sort_block<T, TPTG, TN, STABLE>(    \
      const device T*, device T*, device long*, constant int&, constant long2&, constant bool&, uint3, uint3); \
  template[[host_name("mb_sort_block_" #T "_tptg" #TPTG SUFFIX)]] kernel void                                  \
  mb_sort_block<T, uint, TPTG, TN, STABLE>(                                                                    \
      const device T*, device T*, device uint*, constant int&, constant long2&, constant bool&, uint3, uint3); \
  template [[host_name("mb_sort_block_" #T "_tptg" #TPTG "_u16" SUFFIX)]]                                      \
  kernel void mb_sort_block<T, ushort, TPTG, TN, STABLE>(                                                      \
      const device T*, device T*, device ushort*, constant int&, constant long2&, constant bool&, uint3, uint3);

#define INSTANTIATE_MERGE(T, TPTG, TN)                                                                                 \
  template [[host_name("mb_merge_" #T "_tptg" #TPTG)]]                                                                 \
  kernel void mb_merge<T, uint, uint, TPTG, TN>(                                                                       \
      const device T*, const device uint*, device T*, device uint*, constant int3&, constant bool&, uint3, uint3);     \
  template [[host_name("mb_merge_final_" #T "_tptg" #TPTG)]]                                                           \
  kernel void mb_merge<T, uint, long, TPTG, TN>(                                                                       \
      const device T*, const device uint*, device T*, device long*, constant int3&, constant bool&, uint3, uint3);     \
  template [[host_name("mb_merge_" #T "_tptg" #TPTG "_u16")]]                                                          \
  kernel void mb_merge<T, ushort, ushort, TPTG, TN>(                                                                   \
      const device T*, const device ushort*, device T*, device ushort*, constant int3&, constant bool&, uint3, uint3); \
  template [[host_name("mb_merge_final_" #T "_tptg" #TPTG "_u16")]]                                                    \
  kernel void mb_merge<T, ushort, long, TPTG, TN>(                                                                     \
      const device T*, const device ushort*, device T*, device long*, constant int3&, constant bool&, uint3, uint3);
