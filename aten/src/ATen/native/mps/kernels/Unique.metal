...
#define REGISTER_UNIQUE_FOR_T(T, NAME)                                \
  ...
  unique_mark_boundaries<T>(                                          \
      ...
      constant ulong& numel [[buffer(2)]],                            \
      uint tid [[thread_position_in_grid]]);                          \
  template [[host_name("unique_emit_" #NAME "_int")]] kernel void     \
  unique_emit<T, int>(                                                \
      constant T * sorted [[buffer(0)]],                              \
      constant int* mask [[buffer(1)]],                               \
      ...
      constant ulong& numel [[buffer(5)]],                            \
      uint tid [[thread_position_in_grid]]);                          \
  template [[host_name("unique_emit_" #NAME "_long")]] kernel void    \
  unique_emit<T, long>(                                               \
      constant T * sorted [[buffer(0)]],                              \
      constant int* mask [[buffer(1)]],                               \
...
template [[host_name("unique_inverse_int")]] kernel void unique_inverse<int>(
    constant long* sort_idx [[buffer(0)]],
...
template [[host_name("unique_inverse_long")]] kernel void unique_inverse<long>(
    constant long* sort_idx [[buffer(0)]],
