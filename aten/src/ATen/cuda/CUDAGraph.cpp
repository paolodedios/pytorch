...
std::function<bool(cudaStream_t)> CUDAGraph::create_allocate_filter<cudaStream_t>() const {
  return [this](cudaStream_t stream) {
    auto capture_id_opt = currentStreamCaptureId(stream);
...
std::function<bool(c10::Stream)> CUDAGraph::create_allocate_filter<c10::Stream>() const {
  return [this](c10::Stream stream) {
    ...
    auto capture_id_opt = currentStreamCaptureId(cuda_stream);
    return capture_id_opt.has_value() && capture_id_opt.value() == capture_id_;
  };
}
...
void CUDAGraph::capture_begin(MempoolId_t pool/*=0*/, cudaStreamCaptureMode capture_mode) {
  ...
  // Register this graph in the global map
  {
    std::lock_guard<std::mutex> lock(capture_id_to_graph_mutex);
    capture_id_to_graph[capture_id_] = this;
  }
